...
(function() {
  var appName = 'Data Wrangling Studio';
  ...
  function slugifySection(sectionName) {
    return String(sectionName || 'unknown')
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, '-')
      .replace(/^-+|-+$/g, '') || 'unknown';
  }
  ...
  function trackSectionView(sectionName, navigationType) {
    ...
    var sectionSlug = slugifySection(section);

    safeTrack('page_view', {
      page_title: appName + ' - ' + section,
      page_path: '/' + sectionSlug,
      page_location: window.location.href.split('#')[0] + '#' + sectionSlug,
