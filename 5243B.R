...
app_title <- "Data Wrangling Studio B"

# `%||%` is a small helper for Shiny inputs that may be NULL or empty.
`%||%` <- function(x, y) {
  if (is.null(x) || length(x) == 0) y else x
}

ga_measurement_id <- "G-P7D6PRLLB7"
ga_experiment_name <- "ui_layout_swap_test"
ga_experiment_variant <- "B_swapped_layout"
ga_app_version <- "new_project_app_b"
ga_tracking_js <- sprintf(
  "
...
(function() {
  var appName = 'Data Wrangling Studio B';
  var experimentName = '%s';
  var experimentVariant = '%s';
  var appVersion = '%s';
  ...
  function safeTrack(eventName, params) {
    ...
    params.experiment_variant = experimentVariant;
    params.app_version = appVersion;
    window.gtag('event', eventName, params);
  }

  function detectActiveSection() {
    var activeLink = $('.navbar-nav li.active a, .navbar-nav .nav-link.active').first();

    if (activeLink.length) {
      return $.trim(activeLink.text()) || 'User Guide';
    }

    return 'User Guide';
  }

  function buildVariantPath(sectionName) {
    return '/experiments/' +
      slugify(experimentName) + '/' +
      slugify(experimentVariant) + '/' +
      slugify(sectionName);
  }

  function trackSectionView(sectionName, navigationType) {
    var section = sectionName || detectActiveSection();

    safeTrack('page_view', {
      page_title: appName + ' - ' + section + ' [' + experimentVariant + ']',
      page_path: buildVariantPath(section),
      page_location: window.location.href.split('#')[0] + '#' + slugify(section),
      section_name: section,
      navigation_type: navigationType || 'app_load'
    });
  ...
  function flushSectionTime(trigger) {
    if (!currentSection) {
      return;
    }

    var elapsed = Date.now() - sectionEnteredAt;
    ...
    var payload = {
      ...
      time_source: trigger || 'navigation',
    ...
    safeTrack('section_time_spent', payload);
    sectionEnteredAt = Date.now();
  }

  function beginSection(sectionName, navigationType) {
    currentSection = sectionName || detectActiveSection();
    sectionEnteredAt = Date.now();
    trackSectionView(currentSection, navigationType);
  }
  ...
  function trackExit() {
    ...
    exitTracked = true;
    flushSectionTime('app_exit');
    safeTrack('app_exit', {
      last_section: currentSection || detectActiveSection(),
      session_length_msec: Date.now() - sessionStartedAt,
  ...
  $(document).on('shiny:connected', function() {
    setTimeout(function() {
      beginSection(detectActiveSection(), 'app_load');
      safeTrack('ab_test_assignment', {
        section_name: currentSection,
        assignment_source: 'hardcoded_variant'
      });
    }, 250);
  });

  $(document).on('shown.bs.tab', 'a[data-toggle=\"tab\"], a[data-bs-toggle=\"tab\"]', function(event) {
    var nextSection = $.trim($(event.target).text()) || detectActiveSection();

    if (currentSection && nextSection !== currentSection) {
      flushSectionTime('tab_change');
    }

    if (nextSection !== currentSection) {
      beginSection(nextSection, 'tab_switch');
    }
  });

  $(document).on('click', '#load_builtin, #add_feature, #reset_features, #download_processed_data, button[id^=\"del_feat_\"]', function() {
    var id = this.id || 'unknown';
    var labelMap = {
      load_builtin: 'load_builtin_dataset',
      add_feature: 'add_feature',
      reset_features: 'reset_features',
      download_processed_data: 'download_processed_data'
    };
    var elementName = labelMap[id] || (id.indexOf('del_feat_') === 0 ? 'delete_feature' : ($.trim($(this).text()) || id));

    safeTrack('cta_click', {
      element_id: id,
      element_name: elementName,
      section_name: currentSection || detectActiveSection()
    });

    if (id === 'download_processed_data') {
      safeTrack('download_request', {
        section_name: currentSection || detectActiveSection()
      });
    }
  });
  ...
  $(document).on('change', '#upload_file', function() {
    var selectedFile = this.value ? this.value.split('\\\\').pop() : '';
    var extension = selectedFile.indexOf('.') > -1 ? selectedFile.split('.').pop().toLowerCase() : 'unknown';

    safeTrack('task_started', {
      task_name: 'upload_dataset',
      dataset_source: 'file_upload',
  ...
  window.addEventListener('pagehide', trackExit, { capture: true });
  window.addEventListener('beforeunload', trackExit, { capture: true });

  if (window.Shiny && typeof window.Shiny.addCustomMessageHandler === 'function') {
    window.Shiny.addCustomMessageHandler('ga_event', function(payload) {
      ...
      if (!params.section_name) {
        params.section_name = currentSection || detectActiveSection();
      }

      safeTrack(payload.name, params);
    });
  }
})();
",
...
metric_spec <- function(title, value_output_id, subtitle = NULL, icon_char = NULL) {
  list(
    title = title,
    value_output_id = value_output_id,
    subtitle = subtitle,
    icon_char = icon_char
  )
}

metric_cards_ui <- function(specs) {
  cards <- lapply(specs, function(spec) do.call(metric_card, spec))
  div(class = "swapped-metric-stack", cards)
}

swapped_tab_layout <- function(metric_specs, controls, main_content, sidebar_width = 4) {
  ...
  tagList(
    fluidRow(
      column(
        12,
        div(
          class = "section-card swapped-top-controls-card",
          div(class = "swapped-control-grid", controls)
        )
      )
    ),
    fluidRow(
      column(
        sidebar_width,
        div(
          class = "swapped-metric-sidebar",
          metric_cards_ui(metric_specs)
        )
      ),
      column(
        main_width,
...
metric_sidebar_layout <- function(metric_specs, main_content, sidebar_width = 4) {
  main_width <- 12 - sidebar_width
...
.swapped-top-controls-card {
  margin-bottom: 18px;
}
.swapped-control-grid {
  display: grid;
  grid-template-columns: repeat(auto-fit, minmax(240px, 1fr));
  gap: 12px;
  align-items: start;
}
.swapped-control-grid .status-banner {
  grid-column: 1 / -1;
  margin-bottom: 0;
}
.swapped-control-grid .control-group {
  margin-bottom: 0;
  height: 100%;
}
.swapped-metric-sidebar {
  position: sticky;
  top: 14px;
}
.swapped-metric-stack .metric-box {
  min-height: 0;
}
.hero-box {
  margin: 18px 0 22px;
...
ui <- navbarPage(
  ...
  header = tagList(
    tags$head(
      ...
      tags$script(HTML(ga_tracking_js))
    )
  ),
  ...
  tabPanel(
    ...
    swapped_tab_layout(
      metric_specs = list(
        metric_spec("Rows", "metric_rows", icon_char = "\U0001F4CA"),
        metric_spec("Columns", "metric_cols", icon_char = "\U0001F5C2"),
        metric_spec("Missing Values", "metric_missing", icon_char = "\u26A0\uFE0F"),
        metric_spec("Engineered Features", "metric_features", icon_char = "\u2728")
      ),
      controls = tagList(
  ...
  tabPanel(
    ...
    swapped_tab_layout(
      metric_specs = list(
        metric_spec("Rows", "metric_rows_clean", icon_char = "\U0001F4CA"),
        metric_spec("Columns", "metric_cols_clean", icon_char = "\U0001F5C2"),
        metric_spec("Missing Values", "metric_missing_clean", icon_char = "\u26A0\uFE0F"),
        metric_spec("Duplicates", "metric_duplicates_clean", icon_char = "\U0001F4CB")
      ),
      controls = tagList(
  ...
  tabPanel(
    ...
    swapped_tab_layout(
      metric_specs = list(
        metric_spec("Rows", "metric_rows_pre", icon_char = "\U0001F4CA"),
        metric_spec("Columns", "metric_cols_pre", icon_char = "\U0001F5C2"),
        metric_spec("Numeric Columns", "metric_numeric_pre", icon_char = "\U0001F522"),
        metric_spec("Missing Values", "metric_missing_pre", icon_char = "\u26A0\uFE0F")
      ),
      controls = tagList(
  ...
  tabPanel(
    ...
    swapped_tab_layout(
      metric_specs = list(
        metric_spec("Rows", "metric_rows_feat", icon_char = "\U0001F4CA"),
        metric_spec("Columns", "metric_cols_feat", icon_char = "\U0001F5C2"),
        metric_spec("Saved Feature Rules", "metric_feature_rules", icon_char = "\u2728"),
        metric_spec("Missing Values", "metric_missing_feat", icon_char = "\u26A0\uFE0F")
      ),
      controls = tagList(
  ...
  tabPanel(
    ...
    swapped_tab_layout(
      metric_specs = list(
        metric_spec("Rows After Filter", "metric_rows_eda", icon_char = "\U0001F4CA"),
        metric_spec("Columns", "metric_cols_eda", icon_char = "\U0001F5C2"),
        metric_spec("Numeric Columns", "metric_numeric_eda", icon_char = "\U0001F522"),
        metric_spec("Missing Values", "metric_missing_eda", icon_char = "\u26A0\uFE0F")
      ),
      controls = tagList(
  ...
  tabPanel(
    "Export / Download",
    metric_sidebar_layout(
      metric_specs = list(
        metric_spec("Rows", "metric_rows_export", icon_char = "\U0001F4CA"),
        metric_spec("Columns", "metric_cols_export", icon_char = "\U0001F5C2"),
        metric_spec("Engineered Features", "metric_features_export", icon_char = "\u2728"),
        metric_spec("Missing Values", "metric_missing_export", icon_char = "\u26A0\uFE0F")
      ),
      main_content = tagList(
        div(
          class = "section-card",
...
server <- function(input, output, session) {
  ...
  track_ga_event <- function(event_name, ...) {
    params <- list(...)

    keep_param <- function(value) {
      !(is.null(value) || (length(value) == 1 && is.na(value)))
    }

    params <- params[vapply(params, keep_param, logical(1))]
    session$sendCustomMessage("ga_event", list(name = event_name, params = params))
  }
  ...
  observeEvent(input$load_builtin, {
    ...
    track_ga_event(
      "task_completion",
      task_name = "load_builtin_dataset",
      task_status = "success",
    ...
    tryCatch(
      {
        ...
        track_ga_event(
          ...
          dataset_source = "file_upload",
          file_name = info$name,
          file_extension = tolower(tools::file_ext(info$name))
        )
      },
      error = function(e) {
    ...
    lapply(seq_along(recipes), function(i) {
      local({
        ...
        observeEvent(input[[paste0("del_feat_", idx)]], {
          ...
          track_ga_event(
            "task_completion",
            task_name = "delete_feature_recipe",
            task_status = "success",
  ...
  observeEvent(input$add_feature, {
    ...
    if (!(input$feature_col1 %in% numeric_cols)) {
      ...
      track_ga_event(
        "task_error",
        task_name = "add_feature_recipe",
        task_status = "invalid_input",
        error_message = "Invalid primary numeric column."
      )
      return()
    }
    ...
    if (
      ...
      track_ga_event(
        ...
        task_name = "add_feature_recipe",
        task_status = "invalid_input",
        error_message = "Invalid secondary numeric column."
      )
      return()
    }
